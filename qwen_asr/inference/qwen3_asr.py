...
class Qwen3ASRModel:
    ...
    def transcribe(
        ...
        **kwargs
        ...
        raw_outputs = self._infer_asr(chunk_ctx, chunk_wavs, chunk_lang, max_new_tokens, **kwargs)
    ...
    def transcribe_reduced(
        ...
        mnt = self.max_new_tokens
        if max_new_tokens is not None:
            mnt = max_new_tokens

        total = 0
        for i in range(0, len(texts), batch_size):
            inputs = all_inputs[total]
            inputs = inputs.to(self.model.device).to(self.model.dtype)
            text_ids = self.model.generate(**inputs, max_new_tokens=mnt, **kwargs)
    ...
    def _infer_asr(
        ...
        **kwargs
        ...
        if self.backend == "transformers":
            return self._infer_asr_transformers(contexts, wavs, languages, max_new_tokens, **kwargs)
