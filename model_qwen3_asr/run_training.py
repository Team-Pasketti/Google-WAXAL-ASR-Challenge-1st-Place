...
def patch_outer_forward(model):
    ...
    def forward(
        ...
        **kwargs,
        ...
        try:
            # print(input_ids.shape, attention_mask.shape, input_features.shape, feature_attention_mask.shape, labels.shape)
            res = self.thinker.forward(
                input_ids=input_ids,
...
class DataCollatorForQwen3ASRFinetuning:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        for path, audio_array in zip(audio_paths, audios):
            duration = len(audio_array) / self.sampling_rate
            if duration > max_duration_sec:
                print(f"\nLong audio: {path} ({duration:.2f} sec)")
...
def parse_args():
    ...
    p.add_argument("--train_file", type=str, default=root_path + "input/train_qwen.jsonl")
    ...
    p.add_argument("--save_total_limit", type=int, default=15)
...
class ValidCheckpointCallback(TrainerCallback):
    def on_save(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        print(f"\n[ValidCheckpointCallback] Checkpoint was saved: {ckpt_dir}")
        print("[ValidCheckpointCallback] Run validation...")

        score_wer, score_cer = valid_checkpoint(
            ckpt_dir,
            "../../input/valid_qwen.jsonl"
        )
        try:
            os.rename(ckpt_dir, ckpt_dir + '_wer_{:.4f}_cer_{:.4f}'.format(score_wer, score_cer))
        except Exception as e:
            print('Error', str(e))


def freeze_qwen3_asr_exact(model, unfreeze_llm_layers=4, unfreeze_encoder_layers=2):
    TOTAL_AUDIO_LAYERS = 24
    TOTAL_LLM_LAYERS = 28

    audio_start_unfreeze = TOTAL_AUDIO_LAYERS - unfreeze_encoder_layers
    llm_start_unfreeze = TOTAL_LLM_LAYERS - unfreeze_llm_layers
    ...
    for name, param in model.named_parameters():
        ...
        if "audio_tower.proj1" in name or "audio_tower.proj2" in name:
            param.requires_grad = True

        elif "audio_tower.layers." in name:
            layer_idx = int(name.split("audio_tower.layers.")[1].split(".")[0])
            if layer_idx >= audio_start_unfreeze:
                param.requires_grad = True

        elif "model.layers." in name:
            layer_idx = int(name.split("model.layers.")[1].split(".")[0])
            if layer_idx >= llm_start_unfreeze:
                param.requires_grad = True
...
def main():
    ...
    # model = freeze_qwen3_asr_exact(model, unfreeze_llm_layers=20, unfreeze_encoder_layers=20)
